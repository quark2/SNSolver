                    ...
                    scalar *psU, scalar *psD, int nNX)
{
    ...
    for ( i = 0 ; i < nNumLv ; i++ ) {
        sDiv += arrsCoeff[ i ] * psD[ nNX - 2 - i ];
        sNum += arrsCoeff[ i ] * psU[ nNX - 1 - i ];
        //printf("%13.8f, %13.8f, %13.8f\n", arrsCoeff[ i ], psD[ nNX - 2 - i ], psU[ nNX - 1 - i ]);
    }
